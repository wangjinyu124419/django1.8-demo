...
(function namespacing($) {
	...
	$(document).ready(function() {
		...
		$('#changelist li').click(function(e) {
			...
			admin_base_url = window.location.pathname;
			
			// published checkbox
			if(jtarget.hasClass("publish-checkbox")) {
				pageId = jtarget.attr("name").split("status-")[1];
				reloadItem(jtarget, admin_base_url + "lcx/" + pageId + "/change-status/", {1:1}, refreshIfChildren(pageId));
				e.stopPropagation();
				return false;
			}
			...
			if(jtarget.hasClass("navigation-checkbox")) {
				...
				reloadItem(jtarget, admin_base_url + "lcx/" + pageId + "/change-navigation/", { 1:1 });
				e.stopPropagation();
				return true;
			}
			...
			if(jtarget.hasClass("publish")) {
				...
				reloadItem(jtarget, admin_base_url + "lcx/" + pageId + "/publish/?node=1", {}, refreshIfChildren(pageId));
				e.stopPropagation();
				return false;
			}
			...
			if(jtarget.hasClass("closed")) {
				...
				if(jtarget.find('ul > li').length == 0 && !jtarget.hasClass("loading")) {
					...
					$.get(admin_base_url + "lcx/" + pageId + "/descendants/", {}, function(r, status) {
